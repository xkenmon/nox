  ...
  public void start() {
    ...
    try {
      ...
      serverBootstrap.group(bossGroup, workerGroup)
          ...
          .option(ChannelOption.CONNECT_TIMEOUT_MILLIS, configuration.getTimeoutInSecond() * 1000)
          .childHandler(new SSocksServerChannelInitializer(
              configuration.getPortPasswordPairs(),
              configuration.getMethod()));

      if (bossGroup instanceof EpollEventLoopGroup && configuration.getFastOpen() > 0) {
        serverBootstrap.option(EpollChannelOption.TCP_FASTOPEN, configuration.getFastOpen());
        log.info("TCP fast open enabled.");
      }
